...
for c, chan in enumerate(args.channels):
    ...
    axs[0].plot(times, np.mean(decoding[chan], 0), color=colors[c], linewidth=3)

    # Plot the peak time point
    peak = times[np.argsort(np.mean(decoding[chan], 0))[::-1][0]]
    text = str(int(peak * 1000)) + ' ms'
    max_decoding = max(np.mean(decoding[chan], 0))
    axs[0].text(peak, max_decoding+0.015, text, color='k', ha='center')
    #axs[0].plot([peak, peak], [max_decoding, -100], '--', linewidth=3, color=colors[0],
    #    alpha=.5)
...
axs[0].set_ylim(bottom=46, top=100)

# Save the figure
file_name = os.path.join(save_dir, 'decoding_channels-'+'-'.join(args.channels)+
    '.svg')
