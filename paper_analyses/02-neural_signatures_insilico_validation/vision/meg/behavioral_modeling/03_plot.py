...
save_dir = os.path.join(args.berg_dir, 'neural_signatures_insilico_validation',
    'vision', 'meg', 'behavioral_modeling', 'plots', args.encoding_model)
...
# =============================================================================
# Plot the MEG pairwise decoding results
# =============================================================================
...
for c, sensor in enumerate(args.sensors):

    # Plot the RSA subject-average results
    axs[0].plot(times, np.mean(decoding[sensor], 0), color='k', linewidth=2)

    # Plot the peak time point
    peak = times[np.argmax(np.mean(decoding[sensor], 0))]
    max_dec = max(np.mean(decoding[sensor], 0))
    axs[0].scatter(peak, max_dec, color='k', s=200, marker='o', edgecolors='k',
        linewidths=1, zorder=3)
    ci_low = peak - ci_peak_latency_ci_decoding[sensor][0]
    ci_up = ci_peak_latency_ci_decoding[sensor][1] - peak
    conf_int = np.reshape(np.append(ci_low, ci_up), (-1,1))
    axs[0].errorbar(peak, max_dec, xerr=conf_int, fmt="none", ecolor='k',
        elinewidth=1, capsize=3)

    # Plot the confidence intervals
    axs[0].fill_between(times, ci_decoding[sensor][1], ci_decoding[sensor][0],
        color='k', alpha=.1)
...
for c, sensor in enumerate(args.sensors):
    ...
    # Plot the RSA subject-average results
    axs[0].plot(times, np.mean(rsa[sensor], 0), color='k', linewidth=2)

    # Plot the peak time point
    peak = times[np.argmax(np.mean(rsa[sensor], 0))]
    max_rsa = max(np.mean(rsa[sensor], 0))
    axs[0].scatter(peak, max_rsa, color='k', s=200, marker='o', edgecolors='k',
        linewidths=1, zorder=3)
    ci_low = peak - ci_peak_latency_ci_rsa[sensor][0]
    ci_up = ci_peak_latency_ci_rsa[sensor][1] - peak
    conf_int = np.reshape(np.append(ci_low, ci_up), (-1,1))
    axs[0].errorbar(peak, max_rsa, xerr=conf_int, fmt="none", ecolor='k',
        elinewidth=1, capsize=3)

    # Plot the confidence intervals
    axs[0].fill_between(times, ci_rsa[sensor][1], ci_rsa[sensor][0],
        color='k', alpha=.1)
