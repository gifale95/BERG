...
silico MEG responses for 400 ImageNet images (40 objects categories, with 10
...
# =============================================================================
# Generate the in silico MEG responses for the stimulus images
# =============================================================================
meg = berg.encode(model, images)
...
# (Images × Images × MEG time points)
decoding_exemplars = np.zeros((len(meg), len(meg), len(times)),
    dtype=np.float32)

# Loop over MEG time points and image exemplars
...
# =============================================================================
# Results array of shape:
# (Object categories × Object categories × MEG time points)
object_cats = 40
exemplars_per_cat = 10
decoding_objects = np.zeros((object_cats, object_cats, len(times)),
    dtype=np.float32)

# Loop over MEG time points and object categories
for t in tqdm(range(len(times))):
    for c1 in range(object_cats):
        ...
        meg_cond_1 = meg[idx_c1_start:idx_c1_end,:,:,t]
        ...
        for c2 in range(c1):
            ...
            meg_cond_2 = meg[idx_c2_start:idx_c2_end,:,:,t]
            ...
            # exemplars)
            scores = []
            for e in range(len(meg_cond_1)):

                # Define the train exemplars
                meg_cond_1_train_ex = np.delete(meg_cond_1, e, 0)
                meg_cond_2_train_ex = np.delete(meg_cond_2, e, 0)

                # Loop over MEG repeats (to reduce false positives, the SVMs
                # should be trained and tested on independent in silico MEG
                # response repeats, that is, in silico MEG repeats generated
                # using different encoding models)
                for r in range(meg_cond_1.shape[1]):

                    # Define the train repeats
                    meg_cond_1_train = np.delete(meg_cond_1_train_ex, r, 1)
                    meg_cond_2_train = np.delete(meg_cond_2_train_ex, r, 1)
                    meg_cond_1_train = np.reshape(meg_cond_1_train,
                        (-1, len(kept_ch_names)))
                    meg_cond_2_train = np.reshape(meg_cond_2_train,
                        (-1, len(kept_ch_names)))
                    X_train = np.append(meg_cond_1_train, meg_cond_2_train, 0)

                    # Define the test partition
                    X_test = np.append(np.expand_dims(meg_cond_1[e,r], 0),
                        np.expand_dims(meg_cond_2[e,r], 0), 0)
...
# Results array of shape: (MEG time points)
decoding_animacy = np.zeros((len(times)), dtype=np.float32)

# Loop over MEG time points
for t in tqdm(range(len(times))):

    # Select the animacy data
    meg_animate = meg[animate_conditions,:,:,t]
    meg_inanimate = meg[inanimate_conditions,:,:,t]
    ...
    for c1 in tqdm(range(object_cats//2)): # Half of the total objects are animate
        ...
        idx_c1_start = c1 * exemplars_per_cat
        idx_c1_end = idx_c1_start + exemplars_per_cat
        meg_animate_train_cat = np.delete(
            meg_animate, np.arange(idx_c1_start, idx_c1_end), 0)
        ...
        for c2 in range(object_cats//2): # Half of the total objects are inanimate
            ...
            idx_c2_start = c2 * exemplars_per_cat
            idx_c2_end = idx_c2_start + exemplars_per_cat
            meg_inanimate_train_cat = np.delete(
                meg_inanimate, np.arange(idx_c2_start, idx_c2_end), 0)
            ...
            # encoding models)
            for r in range(meg_animate.shape[1]):

                # Define the train repeats
                meg_animate_train = np.delete(meg_animate_train_cat, r, 1)
                meg_inanimate_train = np.delete(meg_inanimate_train_cat, r, 1)
                meg_animate_train = np.reshape(meg_animate_train,
                    (-1, len(kept_ch_names)))
                meg_inanimate_train = np.reshape(meg_inanimate_train,
                    (-1, len(kept_ch_names)))
                X_train = np.append(meg_animate_train, meg_inanimate_train, 0)
                # Define the test partition
                X_test = np.append(
                    meg_animate[idx_c1_start:idx_c1_end,r],
                    meg_inanimate[idx_c2_start:idx_c2_end,r], 0)
...
n_components = 2
meg_mds = np.zeros((len(meg), n_components, len(times)), dtype=np.float32)
...
for t in tqdm(range(len(times))):
    ...
    meg_mds[:,:,t] = embedding.fit_transform(np.mean(meg[:,:,:,t], 1))
...
save_dir = os.path.join(args.berg_dir, 'neural_signatures_insilico_validation',
    'vision', 'meg', 'object_categorization', 'pairwise_decoding',
    args.encoding_model)
os.makedirs(save_dir, exist_ok=True)

file_name = 'pairwise_decoding_sub-' + format(args.subject, '02') + \
    '_sensors-' + '-'.join(args.sensors) + '.npy'
