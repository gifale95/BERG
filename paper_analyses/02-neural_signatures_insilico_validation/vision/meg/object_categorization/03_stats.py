...
for s, sub in enumerate(args.subjects):
    ...
    data_dir = os.path.join(args.berg_dir,
        ...
        'pairwise_decoding_sub-'+format(sub,'02')+'_sensors-'+
    ...
    if s == 0:
        meg = np.mean(results['meg'], 1)
    else:
        meg = np.append(meg, np.mean(results['meg'], 1), 1)
    ...
    meg_mds_single_sub.append(results['meg_mds'])
...
meg_mds_single_sub = np.asarray(meg_mds_single_sub)
...
# =============================================================================
# Perform MDS on the MEG responses of each time point
# =============================================================================
...
for t in tqdm(range(len(times))):
    ...
    meg_mds[:,:,t] = embedding.fit_transform(meg[:,:,t])
...
results = {
    'meg_mds': meg_mds,
    'meg_mds_single_sub': meg_mds_single_sub,
...
save_dir = os.path.join(args.berg_dir, 'neural_signatures_insilico_validation',
    'vision', 'meg', 'object_categorization', 'stats', args.encoding_model)
