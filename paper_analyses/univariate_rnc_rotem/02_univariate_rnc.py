...
for roi in rois:
    ...
    if args.cv == 0:
        ...
        fmri = np.delete(fmri, args.cv_subject-1, 0)
        fmri_mean[roi] = np.mean(fmri, 0)
    del fmri
...
idx_bad_1 = np.where(fmri_mean[roi_1] < base[roi_1])[0]
idx_bad_2 = np.where(fmri_mean[roi_2] < base[roi_2])[0]
...
for i, cat in enumerate(idx_cat_best):
    ...
    # Select the best N image examplars from each category and neural control
    # condition
    controlling_images[classes[cat][0]] = {
