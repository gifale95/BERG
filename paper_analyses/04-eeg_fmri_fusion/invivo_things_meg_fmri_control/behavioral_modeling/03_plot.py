"""Plot the RSA results between t-fMRI responses and behavioral embeddings.
...
save_dir = os.path.join(args.berg_dir, 'eeg_fmri_fusion',
    'invivo_things_meg_fmri_control', 'behavioral_modeling', 'plots')
...
ci_rsa_peak_lat = results['ci_rsa_peak_lat']
times = results['times']
...
for r, roi in enumerate(rois):
    ...
    plt.plot(times, np.mean(rsa[roi], 0),
        color=colors[r], linewidth=2, label=roi)

    # Plot the CIs
    plt.fill_between(times, ci_rsa[roi][1],
        ci_rsa[roi][0], color=colors[r], alpha=.1)
