...
for ms, msub in enumerate(tqdm(args.meg_subjects)):
    ...
    for stim in unique_test_stimuli:
        ...
        meg_test_sub.append(meg_all[idx].mean(0))
    ...
    if ms == 0:
        ...
        meg = np.append(meg, meg_test_sub, 1)
    del meg_all, meg_test_sub
...
for r, roi in enumerate(tqdm(args.rois)):
    ...
    # (100 Image conditions, N ROI voxels, 140 MEG time points)
    tfmri[roi] = np.zeros((len(unique_test_stimuli), len(voxels_keep),
    ...
    for roi_source in args.rois:
        if roi_target != roi_source:
            ...
            for t in tqdm(range(idx_t_start, len(times))):
                ...
                rsm_roi_target = np.reshape(
                    roi_rsms[roi_target][:,t], (-1, 1))
                rsm_roi_target_past = np.mean(
                    roi_rsms[roi_target][:,idx_onset:idx_offset],
                    1, keepdims=True)
                rsm_roi_source_past = np.mean(
                    roi_rsms[roi_source][:,idx_onset:idx_offset],
                    1, keepdims=True)
                ...
                if args.regression == 'linear':
                    ...
                    reg_full = RidgeCV(alphas=alphas, cv=None,
                        alpha_per_target=True)
                reg_reduced.fit(rsm_roi_target_past, rsm_roi_target)
                reg_full.fit(np.append(rsm_roi_target_past,
                    rsm_roi_source_past, 1), rsm_roi_target)
                ...
                u_full = np.mean((reg_full.predict(np.append(
                    rsm_roi_target_past, rsm_roi_source_past,
                    1)) - rsm_roi_target) ** 2)

                # Adjust the MSE scores for the number of predictors in
                # the models
                n = len(rsm_roi_target)
                ...
                del rsm_roi_target, rsm_roi_target_past, \
                    rsm_roi_source_past, reg_reduced, reg_full, \
                    u_reduced, u_full
