...
tfmri = np.zeros((len(eeg), n_vertex, eeg.shape[3]), dtype=np.float32)
...
for t in tqdm(range(eeg.shape[3])):
    ...
    # Assign each vertex to a different EEG repeat
    vertex_repeat_assignment = np.random.randint(0, eeg.shape[1],
        size=n_vertex)

    # Loop across EEG repeats
    for r in range(eeg.shape[1]):

        # Get the vertices assigned to the current repeat
        assigned_vertices = np.where(vertex_repeat_assignment == r)[0]

        # Instantiate the fusion regression model
        reg = LinearRegression()
        reg.coef_ = reg_param['coef_'][assigned_vertices]
        reg.intercept_ = reg_param['intercept_'][assigned_vertices]
        reg.n_features_in_ = reg_param['n_features_in_']

        # Generate the t-fMRI responses
        tfmri[:,assigned_vertices,t] = reg.predict(eeg[:,r,:,t])
