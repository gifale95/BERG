...
----------
subjects : list
    List of subject identifiers. Valid subject identifiers are integers from 1
    to 8.
hemispheres : list
...
parser.add_argument('--subjects', default=[1, 2, 5, 7], type=list)
...
for fs, fsub in enumerate(tqdm(args.subjects)):
...
for r, roi in enumerate(rois):
    ...
    corr_tfmri_fmri_roi[roi] = np.zeros((len(args.subjects), n_time),
        dtype=np.float32)

    # Loop across subjects and hemispheres
    for fs, fsub in enumerate(args.subjects):
        for h, hemi in enumerate(args.hemispheres):
