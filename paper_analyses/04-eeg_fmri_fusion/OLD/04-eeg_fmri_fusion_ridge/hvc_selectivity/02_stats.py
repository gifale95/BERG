    ...
    for h, hem in enumerate(args.hemispheres):
        ...
        if fs == 0:
            for key in categories:
                ...
                # (N fMRI Subjects, 163,842 Vertices, 4 EEG repeats, 140 Time points)
                tfmri[key][hem] = np.zeros((len(args.fmri_subjects), n_vertex,
                    n_eeg_rep, len(times)), dtype=np.float32)

        # Load the t-fMRI responses
        file_name = f'tfmri_sub-{fsub:02d}_hemi-{hem}.npy'
        tfmri_path = os.path.join(args.berg_dir, 'eeg_fmri_fusion_ridge',
        ...
        for key in tfmri.keys():
            tfmri[key][hem][fs] = tfmri_sub[key]
        del tfmri_sub
...
save_dir = os.path.join(args.berg_dir, 'eeg_fmri_fusion_ridge', 'hvc_selectivity',
