...
register_model(
    ...
    module_path="berg.models.fmri.mosaic_CNN8_multihead_subAll_verticesVisual",
...
class FMRIEncodingModel(BaseModelInterface):
    ...
    def _prepare_vertex_mask(self):
        ...
        Mapping workflow:
        1. Get vertex mapping (7831 → 91k): Load GlasserGroups 1-5 to obtain the 7,831 
        ...
        2. Load glasser_group_id if needed: Array of shape (7831,) indicating which 
        GlasserGroup (1-5) each prediction vertex belongs to.
        ...
        if self.roi_list is not None:
            ...
            roi_dict = metadata[dataset_name][subject_key]["roi"]
        ...
        # Add glasser_group selection (expand from 7831 to 91k space)
        if self.glasser_group_list is not None:
            ...
            dataset_name = list(metadata.keys())[0]
            subject_key = list(metadata[dataset_name].keys())[0]
            glasser_group_id = metadata[dataset_name][subject_key]["glasser_group_id"]
            
            # For each selected group, find vertices in 7831 space and map to 91k
            for group in self.glasser_group_list:
                # Get vertices in 7831 space that belong to this group
                vertices_in_group_7831 = np.where(glasser_group_id == group)[0]
                # Map to 91k space using vertex_mapping
                vertices_in_group_91k = self.vertex_mapping[vertices_in_group_7831]
                selected_indices_91k.update(vertices_in_group_91k)
        
        # Add direct vertex index selection (expand from 7831 to 91k space)
        if self.vertex_index is not None:
            # voxel_index is in 7831 space, expand to 91k using vertex_mapping
            vertex_indices_7831 = np.where(self.vertex_index)[0]
            vertex_indices_91k = self.vertex_mapping[vertex_indices_7831]
        ...
        **kwargs
        ...
        for subject_str in subjects:
            ...
            file_name = os.path.join(
                ...
                'model-CNN8_multihead_subAll_verticesVisual',
                'metadata',
                dataset_prefix,
