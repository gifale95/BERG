...
noise_ceiling = metadata['noise_ceiling_testset']
...
for roi_name in all_rois:
    # Get ROI indices from metadata
    roi_key = f'roi_{roi_name}'
    
    if roi_key in metadata:
        roi_indices = metadata[roi_key]
        
        if len(roi_indices) > 0:
            # Average correlation across voxels in this ROI
            roi_corr = np.mean(correlation_results[roi_indices])
            roi_nc = np.mean(noise_ceiling[roi_indices])
...
# Plot noise ceiling as individual points/lines per ROI
for i, (x, nc) in enumerate(zip(x_positions, roi_noise_ceilings)):
    ax.plot([x-0.35, x+0.35], [nc, nc], color='gray', linewidth=2.5, alpha=0.7)

# Add one legend entry for noise ceiling
ax.plot([], [], color='gray', linewidth=2.5, alpha=0.7, label='Noise Ceiling')
