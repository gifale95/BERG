...
def save_prepr(args, epoched_test, epoched_train, img_conditions_train,
    ...
    (Image conditions x EEG repetitions x EEG channels x EEG time points).
    Then, the data of both training and testing EEG partitions is saved. This
    code additionally computes the noise ceiling signal-to-noise ratio (NCSNR)
    and noise ceiling of the test data.
    ...
    ### Compute the NCSNR and noise ceiling on the test data ###
    # Reshape the EEG data to (conditions, repeats, features)
    n_images = merged_test.shape[0]
    n_repeats = merged_test.shape[1]
    n_channels = merged_test.shape[2]
    n_timepoints = merged_test.shape[3]
    n_features = n_channels * n_timepoints
    neural_data = np.reshape(merged_test, (n_images, n_repeats,  n_features))
    # Estimate the noise standard deviation (calculate the variance of the
    # responses across the 80 presentations of each test image).
    var = []
    for i in range(n_images):
        var.append(np.nanvar(neural_data[i], axis=0, ddof=1))
    ...
    tot_var_data = np.nanvar(np.reshape(neural_data, (-1, n_features)), axis=0,
        ddof=1)
    ...
    # Reshape the scores to (n_channels, n_timepoints)
    ncsnr = ncsnr.reshape(n_channels, n_timepoints)
    noise_ceiling = noise_ceiling.reshape(n_channels, n_timepoints)
    del merged_test, neural_data
